 ...
 * Return:        output - a pointer to a string of chars that represent the encoded message 
 * This function takes the last bit from each RGB block in each pixel and translates it into message
 */ 
char* decodeImage(Image image);

/* Parameters: image - an image created that is a 2d array of pixels 
               message - a pointer to a string of chars that is the message to be encoded into the image
 * Return:        output - an Image struct that represents the new image with the encoded message
 * This function encodes a message passed in into the image by changing the RGB pixel values of the image
 */ 
Image makeEncodedImage(Image image, char* message);

/* Parameters: length - an int value representing the number of bits to represent a binary (9 in this case)
 ...
 * This function takes in a string of 0s and 1s and translates it into a letter in ascii
 */ 
char binaryToAscii(Binary binary);

/* Parameters: header - a header struct that holds the image height and width
 * Return:        output - a pointer to a string of chars that represents a message to be encoded
 * This function prompts the user to enter a message in or to use a default message to encode into an image
 */ 
